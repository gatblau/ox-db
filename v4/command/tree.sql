...
BEGIN
  ...
  /*
      select ox_get_child_link_ids(parent_link_ids):
        gets an array of Ids of all child links of the specified parent items.
    */
  CREATE OR REPLACE FUNCTION ox_get_child_link_ids(parent_item_ids BIGINT[])
  ...
  ALTER FUNCTION ox_get_child_link_ids(BIGINT[])
    OWNER TO onix;

  /*
      select ox_get_child_link_rule_ids(parent_item_type_ids):
        gets an array of Ids of all child link rules of the specified parent item types.
    */
    CREATE OR REPLACE FUNCTION ox_get_child_link_rule_ids(parent_item_type_ids BIGINT[])
    RETURNS TABLE(ids BIGINT[])
    LANGUAGE 'plpgsql'
    STABLE
  AS $BODY$
  ...
  ALTER FUNCTION ox_get_child_link_rule_ids(BIGINT[])
    OWNER TO onix;
  ...
  */
  ...
  /*
      select ox_get_child_item_type_ids(parent_item_type_ids):
        gets an array of Ids of all child item types of the specified parent item types.
    */
  CREATE OR REPLACE FUNCTION ox_get_child_item_type_ids(parent_item_type_ids INT[])
  ...
  ALTER FUNCTION ox_get_child_item_type_ids(INT[])
    OWNER TO onix;
  ...
  */
  ...
  BEGIN
      child_item_ids := (SELECT ox_get_child_item_ids(parent_item_ids::BIGINT[]))::BIGINT[];
    -- recurse
  ...
  /*
      select ox_get_child_item_type_records(parent_item_type_ids):
        gets set of records containing array of Ids of all child item types in the tree for all tree levels.
    */
  CREATE OR REPLACE FUNCTION ox_get_child_item_type_records(parent_item_type_ids INT[])
    RETURNS TABLE(ids INT[])
    LANGUAGE 'plpgsql'
    STABLE
  AS $BODY$
  ...
  BEGIN
    child_item_type_ids := (SELECT ox_get_child_item_type_ids(parent_item_type_ids::INT[]))::INT[];
    -- recurse
    IF (child_item_type_ids IS NOT NULL) THEN
      RETURN QUERY SELECT child_item_type_ids;
      RETURN QUERY SELECT ox_get_child_item_type_records(child_item_type_ids::INT[]);
    END IF;
  END;
  $BODY$;

  ALTER FUNCTION ox_get_child_item_type_records(INT[])
    OWNER TO onix;
  ...
  */
  ...
  BEGIN
    child_item_ids := (SELECT ox_get_child_item_ids(parent_item_ids::BIGINT[]))::BIGINT[];
    child_link_ids := (SELECT ox_get_child_link_ids(parent_item_ids::BIGINT[]))::BIGINT[];
    -- recurse
  ...
  /*
      select ox_get_child_link_rule_records(parent_item_ids):
        gets set of records containing an array of Ids of all child link rules in the tree for all tree levels.
    */
  CREATE OR REPLACE FUNCTION ox_get_child_link_rule_records(parent_item_ids BIGINT[])
    RETURNS TABLE(ids BIGINT[])
    LANGUAGE 'plpgsql'
    STABLE
  AS $BODY$
  ...
  BEGIN
    ...
    IF (child_link_rule_ids IS NOT NULL) THEN
      ...
      RETURN QUERY SELECT ox_get_child_link_rule_records(child_item_type_ids::BIGINT[]);
    END IF;
  END;
  $BODY$;

  ALTER FUNCTION ox_get_child_link_rule_records(bigint[])
    OWNER TO onix;
